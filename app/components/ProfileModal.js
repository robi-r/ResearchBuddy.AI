...
import { User, Award, Shield, FileText, Compass, Sparkles, X, Brain } from "lucide-react";
      ...
      <div className="h-full w-full max-w-md bg-white border-l border-slate-200 shadow-2xl flex flex-col justify-between animate-slide-over text-slate-900 overflow-hidden">
        ...
        <div>
          <div className="bg-slate-900 text-white p-6 relative bg-gradient-to-tr from-slate-900 via-indigo-950 to-slate-900">
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-slate-350 hover:text-white bg-white/10 p-1.5 rounded-full border-none cursor-pointer duration-150"
            >
              <X className="w-4 h-4" />
            </button>
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {profile?.subscription_tier !== "scholar" && (
                    <Sparkles className="w-3 w-3 text-amber-400 fill-amber-400" />
                  )}
                </div>
                <h3 className="font-serif text-lg font-black tracking-tight leading-none">
          ...
          {loading ? (
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-slate-705 font-bold leading-relaxed bg-slate-50 border border-slate-100 rounded-xl p-3">
              ...
              <div className="space-y-1.5">
                <span className="text-[9px] font-mono uppercase tracking-widest text-slate-400 font-black flex items-center gap-1">
                  <Award className="w-3.5 h-3.5 text-indigo-505 text-emerald-500" />
              ...
              <div className="space-y-1.5">
                ...
                <div className="text-xs text-indigo-950 font-serif font-black bg-gradient-to-r from-indigo-50/55 to-slate-50 border border-indigo-150/70 rounded-2xl p-4 leading-relaxed italic relative">
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50/50 flex gap-2 w-full">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 hover:text-slate-900 border border-slate-200 text-slate-705 rounded-xl font-sans text-xs font-black transition cursor-pointer"
          >
            Collapse Drawer
          </button>
        </div>
